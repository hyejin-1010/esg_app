...
class _EditFeedScreenState extends State<EditFeedScreen> {
  ...
  void _getData() async {
    feed = await feedController.loadItem(
  ...
  Future<void> _saveMission() async {
    ...
    feed!.imagePathList = paths;
    feed!.content = _contentController.text;

    try {
      final success = await feedController.updateItem(feed!);
      if (!success) {
        Get.snackbar('실패', '게시물 수정에 실패했습니다.', backgroundColor: Colors.red);
        return;
      }
    } catch (error) {
  ...
  Widget _buildImageItem(String type, dynamic image) {
    return Stack(
      children: [
        SizedBox(
          width: 50.0,
          ...
          child: InkWell(
            onTap: () {
              if (type == 'file') {
                ...
                feed!.imagePathList.remove(image);
              }
              setState(() {});
            },
